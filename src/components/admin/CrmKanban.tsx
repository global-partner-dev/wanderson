...
const STAGES: { id: PipelineStage; title: string; hint: string }[] = [
  ...
  },
  {
    ...
    hint: "Clicksign / DocuSign · Stripe Checkout",
  },
  {
    id: "active",
    title: "Active case",
...
const DEMO_LEADS: LeadRow[] = [
  ...
  },
  {
    ...
    nextAction: "Dispatch e-sign + per-party Stripe sessions",
...
function sourceBadge(source: LeadRow["source"]) {
  const map: Record<LeadRow["source"], string> = {
    triage: "border-transparent bg-primary/90 text-primary-foreground",
    ecommerce: "border-transparent bg-info/90 text-info-foreground",
    meta: "border-transparent bg-secondary text-secondary-foreground",
    referral: "border-transparent bg-success/90 text-success-foreground",
  };
  ...
  return <Badge className={cn("shrink-0 text-[10px] font-bold uppercase", map[source])}>{label[source]}</Badge>;
}
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 md:gap-4">
      <div className="flex shrink-0 flex-col gap-3 border-b border-border/80 bg-background/95 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="truncate text-base font-bold text-foreground md:text-lg">Lead pipeline</h2>
      ...
      {view === "kanban" ? (
                      ...
                      >
                        <div className="mb-2 flex flex-wrap items-center gap-2">
                          ...
                          {row.tags.slice(0, 2).map((t) => (
                            <span
                              key={t}
                              className="rounded bg-muted px-2 py-0.5 text-[10px] font-bold uppercase text-muted-foreground"
                            >
                              {t}
                            </span>
                          ))}
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <button
                            type="button"
                            className="flex-1 rounded-lg border border-border bg-background py-2 text-xs font-semibold text-foreground transition hover:bg-muted/80 min-[340px]:flex-none min-[340px]:px-3"
                          >
                            Open record
                          </button>
                          <button
                            type="button"
                            className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-success py-2 text-xs font-semibold text-success-foreground transition hover:opacity-95 min-[340px]:flex-none min-[340px]:px-3"
                          >
        ...
        <div className="custom-scroll min-h-0 flex-1 overflow-auto rounded-xl border border-border bg-card">
          <Table>
            <TableHeader>
